...
| T-006 低进球分类器 (Python 源) | `assets/t006_lowgoal_classifier_v1.pkl` | 0-1 球分类 (训练源模型) |
| T-006 低进球分类器 (JS 上线) | `assets/t006_lowgoal_export.js` | 0-1 球分类 JS 导出 (200树/22维, prediction-service 加载, C-20260819-004) |
| T-006 特征规格 | `assets/t006_feature_spec.js` | 特征名 + 缺失值回退 + 联赛映射 (参考用) |
...
# 3a. ⚠️ 注意: 在受限沙箱环境 (如 trae-agent sandbox) 中启动需禁用 sandbox,
#     因 PM2 守护进程依赖 Windows 命名管道 (IPC) 通信, sandbox 限制会导致 `pm2 start` 挂起。
#     普通终端/生产环境无需此处理。参考: C-20260819-008
