# -*- coding: utf-8 -*-
"""
P1-11 赔率一致性 10 维特征诊断（特征重要性 + 逐维 Leave-One-Out ablation）
==========================================================================
目标: 区分「共识水平」类（市场回声，可能拖累 RPS）与「偏离/离散」类（独立信号，价值）。
      通过 ① XGB/LGB gain 重要性 ② 逐维 LoO 的 RPS 边际 ③ 分组 ablation
      给出「剔除哪些维、保留哪些维」的子集建议，供二次 A/B 决策。

数据/口径: 与生产一致
  - 基线 build_all_features(slim_odds=True, ts_odds=True) = 233 维
  - + 10 维赔率一致性特征 odds_consensus_features.build_odds_consensus_features
  - 切分: 滚动时间切分（无 shuffle，防泄漏）
  - 模型: XGBoost + LightGBM 生产参数 → 50/50 融合
  - RPS 列序 = [客胜(0), 平(1), 主胜(2)]，与 train_models.compute_rps 完全一致

用法: python scripts/_tmp_p111_shap_ablation.py
"""
...
from odds_consensus_features import build_odds_consensus_features, FEATURE_COLS
...
# ---------- 特征分组（依据 P1-11 10 维来源） ----------
GROUP_CONSENSUS = ["mkt_imp_win", "mkt_imp_draw", "mkt_imp_lose", "mkt_return"]  # 共识水平/抽水
GROUP_DEV = ["mkt_dev_win", "mkt_dev_draw", "mkt_dev_lose", "mkt_dev_abs"]        # 偏离度（value gap）
GROUP_DISP = ["mkt_dispersion", "mkt_company_count"]                              # 离散/覆盖（异质性）
...
def gain_table(lgb_m, xgb_m, cols):
    """XGB/LGB gain 重要性，对齐到 cols 顺序，并归一化为百分比。"""
    xgb_g = np.asarray(xgb_m.feature_importances_, dtype=float)  # sklearn: gain, 对齐训练列序
    lgb_g = np.asarray(lgb_m.booster_.feature_importance(importance_type='gain'), dtype=float)

    def idx(c):
        return cols.index(c) if c in cols else -1

    rows = {}
    for c in cols:
        i = idx(c)
        rows[c] = dict(
            xgb=float(xgb_g[i]) if i >= 0 and i < len(xgb_g) else 0.0,
            lgb=float(lgb_g[i]) if i >= 0 and i < len(lgb_g) else 0.0,
        )
    return rows
...
def main():
    print("=" * 80)
    print("[P1-11] 赔率一致性 10 维特征诊断（gain 重要性 + 逐维 LoO ablation）")
    print("=" * 80)
    ...
    print(f"P1-11 赔率一致性特征维度: {cons_feat.shape[1]}")

    X_full = pd.concat([X_base.reset_index(drop=True), cons_feat.reset_index(drop=True)], axis=1)
    y_arr = np.asarray(y)
    ...
    print("\n" + "=" * 80)
    print("[阶段1] full 模型(233+10维) 上 10 维的 gain 重要性")
    print("=" * 80)
    t0 = time.time()
    ...
    gain = gain_table(lgb_full, xgb_full, FEATURE_COLS)
    print("\n   {:20s} {:>12s} {:>12s}".format("特征", "XGB gain%", "LGB gain%"))
    print("   " + "-" * 46)
    for c in FEATURE_COLS:
        print(f"   {c:20s} {gain[c]['xgb']*100:12.3f} {gain[c]['lgb']*100:12.3f}")
    ...
    for drop_c in FEATURE_COLS:
        ...
        print(f"   -{drop_c:20s} RPS={m['rps']:.4f} delta={delta:+.4f} "
              f"Acc={m['acc']:.4f} LL={m['ll']:.4f} DR={m['draw_recall']:.4f}  [{verdict}] "
              f"({time.time()-t0:.1f}s)")

    # ---------- 阶段 3: 分组 ablation ----------
    print("\n" + "=" * 80)
    print("[阶段3] 分组 ablation（单 fold）")
    print("=" * 80)
    groups = {
        "共识水平(im_win/draw/lose+return)": GROUP_CONSENSUS,
        "偏离度(dev_win/draw/lose/abs)": GROUP_DEV,
        "离散覆盖(dispersion/count)": GROUP_DISP,
        "偏离+离散(推荐子集)": GROUP_DEV + GROUP_DISP,
    }
    group_res = {}
    for gname, gcols in groups.items():
        ...
        print(f"   剔除 {gname:32s} → RPS={m['rps']:.4f} delta={delta:+.4f} "
              f"Acc={m['acc']:.4f} LL={m['ll']:.4f} ({time.time()-t0:.1f}s)")

    # ---------- 阶段 4: 最终 2-fold 滚动 A/B（子集 vs full vs 基线） ----------
    print("\n" + "=" * 80)
    print("[阶段4] 最终 2-fold 滚动 A/B：基线 / full(+10) / 偏离+离散子集")
    print("=" * 80)
    subsets = {
        "base(198)": [],
        "full(+10)": FEATURE_COLS,
        "dev+disp(+6)": GROUP_DEV + GROUP_DISP,
        "consensus(+4)": GROUP_CONSENSUS,
    }
    ...
    full_mean = {k: np.nanmean([r[k] for r in final['full(+10)']]) for k in
    ...
    for c in FEATURE_COLS:
        ...
        print(f"   {c:20s} delta_RPS={v['delta']:+.4f}  → {tag}")
    print(f"\n   建议剔除({len(drop)}): {drop}")
